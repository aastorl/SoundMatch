package com.example.soundmatch.data.remote.musicservice

import android.media.MediaPlayer
import android.util.Log
import androidx.lifecycle.ViewModel
import com.example.soundmatch.data.remote.response.AlbumResponse
import com.example.soundmatch.data.remote.response.AlbumsMetadataResponse
import com.example.soundmatch.data.remote.response.ArtistResponse
import com.example.soundmatch.data.remote.response.BrowseCategoriesResponse
import com.example.soundmatch.data.remote.response.EpisodeResponse
import com.example.soundmatch.data.remote.response.EpisodesWithPreviewUrlResponse
import com.example.soundmatch.data.remote.response.FeaturedPlaylistsResponse
import com.example.soundmatch.data.remote.response.NewReleasesResponse
import com.example.soundmatch.data.remote.response.PlaylistItemsResponse
import com.example.soundmatch.data.remote.response.PlaylistsForSpecificCategoryResponse
import com.example.soundmatch.data.remote.response.SearchResultsResponse
import com.example.soundmatch.data.remote.response.ShowResponse
import com.example.soundmatch.data.remote.response.TracksWithAlbumMetadataListResponse
...
/**
 * An enum that contains all the genres supported by the spotify api.
...
fun getUserCountry(): String {
    return Locale.getDefault().country // "ES", "US", "AR", etc.
}
...
interface SpotifyService {
    ...
    suspend fun getAlbumsOfArtistWithId(
        @Path("id") artistId: String,
        @Query("market") market: String = getUserCountry(),
        @Header("Authorization") token: BearerToken,
...
class PlaybackViewModel : ViewModel() {
    ...
    fun playTrack(track: Track) {
        val streamUrl = track.previewUrl // Ensure this matches your Track model
        if (streamUrl.isNullOrEmpty()) {
            Log.e("PlaybackViewModel", "Error: Stream URL is null or empty")
            return
        }

        Log.d("PlaybackViewModel", "Attempting to play: ${track.name}, URL: $streamUrl")
        ...
        try {
            ...
            Log.e("PlaybackViewModel", "Error playing track: ${e.message}")
        }
    }
